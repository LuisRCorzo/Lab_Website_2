	...
	function loadStatsPage(){
		...
		for(row_counter = 2; row_counter < table.rows.length; row_counter++){//Outer for loop iterates over each row
			...
			for(col_counter = 2; col_counter < 4; col_counter++){
				...
				if(col_counter === 2){
					home = parseInt(table.rows[row_counter].cells[col_counter].innerHTML);//Read in a cells current value
				}
				if(col_counter === 3){
					opp = parseInt(table.rows[row_counter].cells[col_counter].innerHTML);//Read in a cells current value
				}
			...
			table.rows[row_counter].cells[4].innerHTML = winner;
...
function loadPlayersPage(){
	...
	for(let i=0; i<players.length; i++){
		...
		var playerField = document.createElement('x');
		playerField.href = "#";
		playerField.className = "playerField";
		playerField.index = i;
		playerField.innerHTML = players[i].name;
		playerField.onclick = function(){switchPlayers(i)};

		playerSelector.appendChild(playerField);
...
function switchPlayers(num){
	...
	let passes = player.pass_yards/player.games_played;
	let rushDistance = player.rushing_yards/player.games_played;
	let receives = player.receiving_yards/player.games_played;
	...
	document.getElementById("rec_img").innerHTML = player.receiving_yards;

	document.getElementById("avg_p_yards").innerHTML = passes.toFixed(2);
	document.getElementById("avg_r_yards").innerHTML = rushDistance.toFixed(2);
	document.getElementById("avg_rec_yards").innerHTML = receives.toFixed(2);
}
